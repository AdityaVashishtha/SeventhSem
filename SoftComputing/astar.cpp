...
int main()
{
  ...
  printf("\nThe Eight Puzzle!\n");
  printf("Enter the matrix values (space seperated)\n");
  int i = 0;
  while(i<9)
	{
	char chr;
	chr = fgetc(stdin);
	if (chr==' ') continue; // for space
	if (chr=='0') block[i] = 8;
	else block[i] = chr - '1';
	i++;
	}
 fgetc(stdin); //flush out the end of line character
  ...
  while(i<9)
	...
	chr = fgetc(stdin);
	if (chr==' ') continue;
	if (chr=='0') goal_block[i] = 8;
	else goal_block[i] = chr - '1';
	i++;
	}
  ...
  top = newelement();
  for(i=0; i<9; i++)    top->block[i] = block[i];
